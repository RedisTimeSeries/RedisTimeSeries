...
- `RETENTION` _retentionPeriod_ - Maximum age for samples compared to highest reported timestamp (in milliseconds).
   ...
   Samples are expired based solely on the difference between their timestamp and the timestamps passed to subsequent [TS.ADD](/commands/ts.add/), [TS.MADD](/commands/ts.madd/), [TS.INCRBY](/commands/ts.incrby/), and [TS.DECRBY](/commands/ts.decrby/) calls.
