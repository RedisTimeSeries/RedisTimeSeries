...
- `LATEST` (since RedisTimeSeries v1.8)
  ...
  When the time series is a compaction: With `LATEST`, TS.RANGE will also report the compacted value of the latest (possibly partial) bucket (given that that bucket start time falls within [fromTimestamp, toTimestamp]). Without `LATEST`, TS.RANGE will not report the latest (possibly partial) bucket. When the series is not a compaction: `LATEST` is ignored.
