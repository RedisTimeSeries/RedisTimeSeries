...
def test_keyspace():
    ...
    with env.getClusterConnectionIfNeeded() as r:
        ...
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.add')
        ...
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester{2}')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.add')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'test_key2{2}')

        # Test INCRBY generate event on key
        r.execute_command("ts.INCRBY", 'tester{2}', "100")
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.incrby')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester{2}')

        # Test DECRBY generate event on key
        r.execute_command("ts.DECRBY", 'tester{2}', "13")
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.decrby')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester{2}')

        # Test DEL generate event on key
        r.execute_command("ts.DEL", 'tester{2}', "100", "100")
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.del')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester{2}')
...
def test_keyspace_create_rules():
    ...
    with env.getClusterConnectionIfNeeded() as r:
        ...
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.create')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_src{2}')

        r.execute_command('TS.CREATE', 'tester_dest{2}')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.create')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_dest{2}')

        r.execute_command('TS.CREATERULE', 'tester_src{2}', 'tester_dest{2}', 'AGGREGATION', 'COUNT', 10)
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.createrule:src')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_src{2}')

        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.createrule:dest')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_dest{2}')

        r.execute_command('TS.DELETERULE', 'tester_src{2}', 'tester_dest{2}')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.deleterule:src')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_src{2}')

        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.deleterule:dest')
        assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_dest{2}')
...
def test_keyspace_rules_send():
    ...
    with env.getClusterConnectionIfNeeded() as r:
        ...
        for i in range(1000):
            ...
            assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'ts.add:dest')
            assert_msg(env, pubsub.get_message(timeout=1), 'pmessage', 'tester_dest{2}')
