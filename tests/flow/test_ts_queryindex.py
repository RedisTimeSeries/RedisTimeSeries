...
def test_label_index():
    ...
    with Env().getClusterConnectionIfNeeded() as r:
        ...
        def assert_data(query, expected_data):
            assert sorted(expected_data) == sorted(env.getConnection(0).execute_command(*query))
        ...
        with pytest.raises(redis.ResponseError):
            env.getConnection(0).execute_command('TS.QUERYINDEX', 'z=', 'x!=2')
        ...
        assert sorted(env.getConnection(0).execute_command('TS.QUERYINDEX', 'generation=x')) == sorted(env.getConnection(0).execute_command('TS.QUERYINDEX',
                                                                                       'generation=(x)'))
        ...
        with pytest.raises(redis.ResponseError):
            assert env.getConnection(0).execute_command('TS.QUERYINDEX', 'generation=x', 'class=(')
        with pytest.raises(redis.ResponseError):
            assert env.getConnection(0).execute_command('TS.QUERYINDEX', 'generation=x', 'class=(ab')
        with pytest.raises(redis.ResponseError):
            assert env.getConnection(0).execute_command('TS.QUERYINDEX', 'generation!=(x,y)')
...
def test_large_key_value_pairs():
    ...
    with Env().getClusterConnectionIfNeeded() as r:
        ...
        for kv_label in kv_labels:
            res = env.getConnection(0).execute_command('TS.QUERYINDEX', kv_label1)
