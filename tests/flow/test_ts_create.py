...
def test_mixed_case_duplicate_policy_and_aggregation():
    ...
    with Env().getClusterConnectionIfNeeded() as r:
        ...
        # ON_DUPLICATE is a second argument token into the same parser, so exercise it
        # too: it must override the series' BLOCK policy for this one sample
        assert r.execute_command('TS.ADD', 'mixed_dp_block', 1000, 7, 'ON_DUPLICATE', 'LaSt')
        assert r.execute_command('TS.RANGE', 'mixed_dp_block', 1000, 1000) == [[1000, b'7']]
