...
def test_resp3():
    ...
    with env.getClusterConnectionIfNeeded() as rc:
        with env.getConnection(1) as r:
            ...
            res = r.execute_command('ts.mget', 'WITHLABELS', 'FILTER', 'name=mush')
...
def test_resp3_mrange():
    ...
    with env.getClusterConnectionIfNeeded() as rc:
        with env.getConnection(1) as r:
            ...
            rc.execute_command('ts.create', t4, 'CHUNK_SIZE', 128, 'LABELS', 'name', 'bush')
            rc.execute_command('ts.create', t5, 'CHUNK_SIZE', 128, 'LABELS', 'name', 'rush')
            rc.execute_command('ts.create', t6, 'CHUNK_SIZE', 128, 'LABELS', 'name', 'rush')

            _insert_data(rc, t3, 1, 10, 2)
            _insert_data(rc, t4, 1, 10, 6)
            _insert_data(rc, t5, 1, 10, 2)
            _insert_data(rc, t6, 1, 10, 6)
