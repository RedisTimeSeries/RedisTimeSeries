...
def test_mget_with_expire_cmd():
    ...
    with Env().getClusterConnectionIfNeeded() as r:
        ...
        while time.time() < (current_ts+10):
            reply = env.getConnection(0).execute_command('TS.MGET', 'FILTER', 'type=DELAYED')
...
def test_mget_cmd():
    ...
    with Env(decodeResponses=True).getClusterConnectionIfNeeded() as r:
        ...
        actual_result = env.getConnection(0).execute_command('TS.MGET', 'FILTER', 'NODATA=TRUE')
        ...
        actual_result = env.getConnection(0).execute_command('TS.MGET', 'FILTER', 'a=1')
        assert sorted(expected_result) == decode_if_needed(sorted(actual_result))
        ...
        actual_result = env.getConnection(0).execute_command('TS.MGET', 'WITHLABELS', 'FILTER', 'a!=1', 'b=1')
        ...
        actual_result = env.getConnection(0).execute_command('TS.MGET', 'WITHLABELS', 'FILTER', 'a=1')
        ...
        actual_result = env.getConnection(0).execute_command('TS.MGET', 'SELECTED_LABELS', 'metric', "new_label",'FILTER', 'a=1')
        assert sorted(expected_result_withlabels) == decode_if_needed(sorted(actual_result))

        # negative test
        assert not env.getConnection(0).execute_command('TS.MGET', 'FILTER', 'a=100')
        assert not env.getConnection(0).execute_command('TS.MGET', 'FILTER', 'k=1')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'filter')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'filter', 'k+1')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'filter', 'k!=5')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'retlif', 'k!=5')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'SELECTED_LABELS', 'filter', 'k!=5')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'SELECTED_LABELS', 'filter', 'k!=5')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'SELECTED_LABELS', 'WITHLABELS', 'filter', 'k!=5')
        with pytest.raises(redis.ResponseError) as excinfo:
            assert env.getConnection(0).execute_command('TS.MGET', 'WITHLABELS', 'SELECTED_LABELS', 'filter', 'k!=5')
...
def test_large_key_value_pairs():
    ...
    with Env().getClusterConnectionIfNeeded() as r:
        ...
        for kv_label in kv_labels:
            res = env.getConnection(0).execute_command('TS.MGET', 'FILTER', kv_label1)
...
def test_latest_flag_mget():
    ...
    with env.getClusterConnectionIfNeeded() as r:
        ...
        res = env.getConnection(0).execute_command('TS.mget', 'FILTER', 'is_compaction=true')
        assert res == [['t2{1}', [], [0, '4']]] or res == [[b't2{1}', [], [0, b'4']]]
        res = env.getConnection(0).execute_command('TS.mget', "LATEST", 'FILTER', 'is_compaction=true')
