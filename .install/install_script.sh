...
#
# Caller may pre-set OSNICK in the environment (e.g. Dockerfile ENV/ARG, CI
# pipeline) to skip auto-detection. Useful on minimal containers where
# `deps/readies/bin/platform` and/or `/etc/os-release` are absent or
# misleading. No validation: an unknown value just falls through
# `is_migrated` to the legacy path, same as an unrecognised auto-detected
# value would. Note: the override only steers the abstract path's match
# against `migrated_osnicks` in dependencies.yaml; the legacy fallback path
# below re-derives `legacy_os` from /etc/os-release regardless.
# ----------------------------------------------------------------------------

OSNICK="${OSNICK:-}"

if [ -n "$OSNICK" ]; then
    # `migrated_osnicks` uses rocky*/alma* — map common RHEL nicknames so callers
    # can pass OSNICK=rhel8 without falling through to a missing legacy script.
    case "$OSNICK" in
        rhel8)  OSNICK=rocky8 ;;
        rhel9)  OSNICK=rocky9 ;;
        rhel10) OSNICK=rocky10 ;;
    esac
    echo "OSNICK=$OSNICK (provided by caller, skipping auto-detect)"
else
    ...
    if [ -z "$OSNICK" ] && [ -r /etc/os-release ]; then
        ...
        case "${ID:-}:${VERSION_ID:-}" in
            ...
            rhel:8*|redhat:8*)       OSNICK="rocky8"          ;;
            rhel:9*|redhat:9*)       OSNICK="rocky9"          ;;
            rhel:10*|redhat:10*)    OSNICK="rocky10"         ;;
...
#     rltest:
#       apt: skip
#       pip: [-r tests/flow/requirements.txt]
#   package_map:
#     <abstract>:
#       <pm>: [pkg1, pkg2]
#       <pm>: skip
...
# Provision `uv` + a project-local venv (see dependencies.yaml `system: uv`).
...
# All PyPI installs from this script use **uv** (`uv pip install`, same tool as
# `uv venv`). They go into $ROOT/venv via `uv pip install --python $ROOT/venv/bin/python`.
#
# We avoid `uv pip install --system`: on RHEL 8, `make bootstrap` passes `sudo` for
# dnf only; running `uv pip` under sudo still made uv resolve against /usr’s
# python3 (3.6), breaking rltest. Venv installs stay `uv` end-to-end and are not sudo’d.
#
# Flow tests: tests/flow/tests.sh prefers $ROOT/venv/bin/python when it has RLTest.
uv_pip_venv() {
    if [ "${1:-}" != install ]; then
        echo "install_script.sh: uv_pip_venv: expected 'install' as first arg, got: $*" >&2
        exit 1
    fi
    shift
    if [ ! -x "$ROOT/venv/bin/python" ]; then
        echo "install_script.sh: missing $ROOT/venv/bin/python (uv venv step failed?)" >&2
        exit 1
    fi
    local -a uvargs=(install --python "$ROOT/venv/bin/python")
    uvargs+=("$@")
    uv pip "${uvargs[@]}"
}
if command -v uv >/dev/null 2>&1; then
    echo "install_script.sh: uv pip install → $ROOT/venv (uv; venv target, not --system)"
    uv_pip_venv install --upgrade pip wheel "setuptools<81"
    while IFS= read -r abs; do
        [ -z "$abs" ] && continue
        pip_args=$(resolve_abstract "$abs" "pip" "$DEPS_YAML")
        [ -z "$pip_args" ] && continue
        # Paths in YAML are relative to $ROOT; callers may cwd elsewhere.
        # shellcheck disable=SC2086
        (cd "$ROOT" && uv_pip_venv install $pip_args)
    done <<EOF
$(extract_flat_list system "$DEPS_YAML")
EOF
    while IFS= read -r rel; do
        [ -z "$rel" ] && continue
        if [ ! -f "$ROOT/$rel" ]; then
            echo "install_script.sh: missing requirement file $ROOT/$rel" >&2
            exit 1
        fi
        uv_pip_venv install -r "$ROOT/$rel"
    done <<EOF
$(extract_flat_list python "$DEPS_YAML")
EOF
else
    echo "install_script.sh: WARNING uv missing; install RLTest via 'make bootstrap'" >&2
fi
