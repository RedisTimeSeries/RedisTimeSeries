...
#
# The value lives in a single source-of-truth field (`redis_ref`) in the RAMP
# manifest, pack/ramp.yml, so it does not have to be duplicated across
...
# Value of the top-level `redis_ref:` key, with inline comment, surrounding
