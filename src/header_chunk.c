...
Chunk_t *Header_NewChunk(size_t size) {
    ...
    chunk->size = size;
    chunk->data = (u_int8_t *)calloc(chunk->size, sizeof(char));
#ifdef DEBUG
...
static ChunkResult Header_ChunkIteratorGetNext(ChunkIter_t *abstractIter, Sample *sample) {
    Header_Iterator *iter = abstractIter;
    HeaderChunk *chunk = iter->chunk;
    if (unlikely(iter->count == 0)) {
        sample->timestamp = iter->prevTS;
        sample->value = iter->prevValue;
        iter->count++;
        return CR_OK;
    } else if (unlikely(iter->count >= chunk->count)) {
        return CR_END;
    } 
    HeaderSample *header = (HeaderSample *)&chunk->data[iter->idx++];
    if (header->zeroTS) {
        sample->timestamp = iter->prevTS;
    } else {
        int64_t dd = 0;
        for (int i = 0; i < header->countTS; ++i) {
            dd += (chunk->data[iter->idx++] << (i * 8));
        }
        if (header->signTS) dd = -dd;
        iter->prevDeltaTs += dd;
        sample->timestamp = iter->prevTS += iter->prevDeltaTs;
    }
    if (header->zeroVal) {
        sample->value = iter->prevValue;
    } else {
        int64_t dd = 0;
        for (int i = 0; i < header->countVal; ++i) {
            dd += (chunk->data[iter->idx++] << (i * 8));
        }
        if (header->signVal) dd = -dd;
        iter->prevValueDelta += dd;
        sample->value = iter->prevValue += iter->prevValueDelta;
    }
    iter->count++;
    return CR_OK;
}
...
static void ensureAddSample(HeaderChunk *chunk, Sample *sample) {
    ...
    if (res != CR_OK) {
        ...
        chunk->data = (u_int8_t *)realloc(chunk->data, chunk->size * sizeof(char));
...
static void trimChunk(HeaderChunk *chunk) {
    ...
    if (excess > 1) {
        ...
        newSize += sizeof(uint64_t) - (newSize % sizeof(uint64_t));
...
ChunkResult Header_AddSample(Chunk_t *chunkExt, Sample *sample) {
    HeaderChunk *chunk = chunkExt;
    if (chunk->count == 0) {
        chunk->baseValue = chunk->prevValue = sample->value;
        chunk->baseTimestamp = chunk->prevTimestamp = sample->timestamp;
        chunk->prevTimestampDelta = 0;
        chunk->prevValueDelta = 0;
    } else {
        if (unlikely(chunk->idx + 9 > chunk->size)) {
            return CR_END;
        }
        HeaderSample *header = (HeaderSample *)&chunk->data[chunk->idx++];
        timestamp_t curDelta = sample->timestamp - chunk->prevTimestamp;
        int64_t doubleDelta = curDelta - chunk->prevTimestampDelta;
        if (!doubleDelta) {
            header->zeroTS = 1;
        } else {
            if (doubleDelta < 0) {
            ...
            while (doubleDelta) {
                chunk->data[chunk->idx++] = doubleDelta & 0xff;
                doubleDelta >>= 8;
                header->countTS++;
            }
        }
        chunk->prevTimestamp = sample->timestamp;
        chunk->prevTimestampDelta = curDelta;

        int64_t valDelta = sample->value - chunk->prevValue;
        int64_t valDoubleDelta = valDelta - chunk->prevValueDelta;
        if (!valDoubleDelta) {
            header->zeroVal = 1;
        } else {
            if (valDoubleDelta < 0) {
                header->signVal = 1;
                valDoubleDelta = - valDoubleDelta;
            }
            while (valDoubleDelta) {
                chunk->data[chunk->idx++] = valDoubleDelta & 0xff;
                valDoubleDelta >>= 8;
                header->countVal++;
            }
        }
        chunk->prevValue = sample->value;
        chunk->prevValueDelta = valDelta;
    }
    chunk->count++;
    return CR_OK;
}
...
double Header_GetLastValue(Chunk_t *chunk) {
    ...
    return ((HeaderChunk *)chunk)->prevValue;
}
...
static void Header_Serialize(Chunk_t *chunk,
    ...
    saveUnsigned(ctx, compchunk->baseValue);
    ...
    saveUnsigned(ctx, compchunk->prevValue);
...
#define COMPRESSED_DESERIALIZE(chunk, ctx, readUnsigned, readStringBuffer, ...)                    \
    do {                                                                                           \
        ...
        compchunk->baseValue = readUnsigned(ctx, ##__VA_ARGS__);                                 \
        ...
        compchunk->prevValue = readUnsigned(ctx, ##__VA_ARGS__);                                 \
                                                                                                    \
        size_t len;                                                                                \
        compchunk->data = (uint8_t *)readStringBuffer(ctx, &len, ##__VA_ARGS__);                  \
