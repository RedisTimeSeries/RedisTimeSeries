...
static void *QueryPredicates_ArgDeserialize_impl(ReaderSerializationCtx *sctx,
    ...
    bool resp3 = expect_resp ? MR_SerializationCtxReadLongLong(sctx, error) : false;
    // check that resp3, if it exists, is false. 1.8 cannot handle it.
    // *error must be NULL here, as ReadLongLong checks that we do not exceed the buffer.
    if (unlikely(expect_resp && resp3 != false)) {
        goto err;
    }
...
static void *QueryPredicates_ArgDeserialize(ReaderSerializationCtx *sctx, MRError **error) {
    return QueryPredicates_ArgDeserialize_impl(sctx, error, false)
               ?: QueryPredicates_ArgDeserialize_impl(sctx, error, true);
}
