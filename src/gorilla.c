...
static inline binary_t readBits(const binary_t *bins,
    ...
    globalbit_t norm_start_pos = start_pos >> 6;
    if (available >= dataLen) {
        return LSB(bins[norm_start_pos] >> lbit, dataLen);
    } else {
        binary_t bin = LSB(bins[norm_start_pos] >> lbit, available);
        bin |= LSB(bins[(norm_start_pos) + 1], dataLen - available) << available;
        return bin;
    }
}
