...
static void *collect_node_results(ExecutionCtx *eCtx, RedisModuleCtx *ctx) {
    ...
    if (len == 0) {
        ...
        RedisModule_ReplyWithError(ctx, "ERR unexpected results from nodes");
        return NULL;
    }
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (nodesResultsType && nodesResultsType != r->recordType) {
            ...
            RedisModule_ReplyWithError(ctx, "ERR mixed node result types");
        ...
        RedisModule_ReplyWithError(ctx, "ERR unexpected record type");
        array_free(nodesResults);
        return NULL;
    }
