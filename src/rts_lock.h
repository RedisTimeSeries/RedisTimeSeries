...
// We are using recursive because writes are serialized and synchronized by redis (libmr support
// only reads). We can implement this cheaply, the need for the is load_rdb which can be called when
// the lock is already taken on replication scenario when the Backup_Globals() is being called
// before.
